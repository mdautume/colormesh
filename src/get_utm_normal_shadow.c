...
void initialize_img_copy_sun_plan_and_visibilities(
        double *img_copy,
        double *sun_plan,
        int *v_cam_visibility,
        int *v_sun_visibility,
        int xywh[4],
        int xywhs[4],
        int nv)
{
    ...
    for (int i = 0; i < nv; i++){
        v_cam_visibility[i] = 0;
        v_sun_visibility[i] = 0;}
}
...
void check_triangle_orientation(
        ...
        struct rpc *huge_rpc,
        double az_el[2],
        int *v_cam_visibility,               // is the vertex seen by camera ?
        int *v_sun_visibility,               // is the vertex seen by sun ?
        double cam_dir[3],
        double sun_dir[3],
        ...
        double *t_angles)                       // triangles angles
{
        ...
        double tri_normal[3] = {0, 0, 0};
        //double sun_dir[3] = {0, 0, 0};
        //double cam_dir[3] = {0, 0, 0};
        ...
        if (scalar_product(tri_normal, cam_dir, 3) < 0)
            for (int i = 0; i < 3; i++)
                v_cam_visibility[triangle_vertices[i]] = 1;

        // check if triangle faces the sun
        if (scalar_product(tri_normal, sun_dir, 3) < 0)
            for (int i = 0; i < 3; i++)
                v_sun_visibility[triangle_vertices[i]] = 1;

        // compute triangle angles (en fait retourne 1 pour chaque angle)
        double angles[3];
        ...
        double *t_angles)                       // triangles angles
{
    for (int t = 0; t < m->nt; t++)
    {
        ...
        check_triangle_orientation(t, triangle_coord_meters, triangle_vertices, m, huge_rpc, az_el, v_cam_visibility, v_sun_visibility, cam_dir, sun_dir, t_normals, t_angles);
    }
}
        ...
        double *sun_plan)                     // image 3*w*h
{
    ...
    double sun_dir[3];
    sun_direction(sun_dir, az_el[0], az_el[1]);
    double el_rad = az_el[1] * M_PI / 180;
...
//            if  (pow(scalar_product(diff, sun_dir, 3),2) < 4*pow(euclidean_norm(sc,2),2))
    if (fabs(sun_plan[3 * (ij[1] * xywhs[2] + ij[0]) + 2] - enz[2]) < (sqrt(2)*0.5/cos(el_rad)))
        for (int i = 0; i < 3; i++){
            if (isnan(enz[i]))
                printf("isnan v %d\n", v);
            out_sun[3 * v + i] = enz[i];
...
void check_vertices_visibility(
        ...
        double *img_copy,                     // image 3*w*h
        double *sun_plan,                     // image 3*ws*hs
        int *v_cam_visibility,               // true if facing the camera
        int *v_sun_visibility,               // true if facing the sun
        double *v_normals,                    // vertices normals
...
int main(int c, char *v[])
{
    ...
    double offset[3] = {xmin-ox, ymin-oy, oz}; // offset to apply (crop + shift)
    ...
    char *filename_sun_visibility = v[13];                     // vertices lightning
    ...
    int *v_cam_visibility = malloc(m->nv * sizeof(int));
    int *v_sun_visibility = malloc(m->nv * sizeof(int));

    initialize_img_copy_sun_plan_and_visibilities(img_copy, sun_plan, 
            v_cam_visibility, v_sun_visibility, xywh, xywhs, m->nv);
...
// get direction vector of the camera (from camera to observer)
    ...
    fill_img_copy_sun_plan_with_highest_point(m, origin, signed_zone, huge_rpc,
            ...
            v_cam_visibility, v_sun_visibility, cam_dir, sun_dir, t_normals, t_angles);
    ...
    iio_write_image_int_vec(filename_sun_visibility, v_sun_visibility, m->nv, 1, 1);

    iio_write_image_int_vec("visibility.tif", v_sun_visibility, m->nv, 1, 1);
